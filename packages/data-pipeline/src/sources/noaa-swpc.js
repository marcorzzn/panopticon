...
function getStormLevel(kp) {
    if (kp >= 9)
        return GeomagneticStormLevel.STORM_G5;
    if (kp >= 8)
        return GeomagneticStormLevel.STORM_G4;
    if (kp >= 7)
        return GeomagneticStormLevel.STORM_G3;
    if (kp >= 6)
        return GeomagneticStormLevel.STORM_G2;
    if (kp >= 5)
        return GeomagneticStormLevel.STORM_G1;
    if (kp >= 4)
        return GeomagneticStormLevel.UNSETTLED;
...
export async function fetchSpaceWeather() {
    ...
    let kpIndex = 1;
    let kpForecast = [1, 2, 1, 3, 2, 1, 2, 1];
...
//# sourceMappingURL=noaa-swpc.js.map